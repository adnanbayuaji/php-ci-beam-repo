...
class User extends CI_Controller
{
    ...
    function tambah()
    {
        ...
        if($this->Menumodel->returnbooladd($this->session->userdata('role'), 'pengguna'))
        {
            //cek dulu admin (kondisi plant)
    ...
    {
        $this->load->model('Menumodel');
        if($this->Menumodel->returnbool($this->session->userdata('role'), 'pengguna'))
        {
            $data['akses']=$this->Menumodel->getstatus($this->session->userdata('role'), 'pengguna');
            $data['menu']=$this->Menumodel->getmenu($this->session->userdata('role'));
    ...
    {
        $this->load->model('Menumodel');
        if($this->Menumodel->returnbooldelete($this->session->userdata('role'), 'pengguna'))
        {
            //meload file model
            $this->load->model('Usermodel');
    ...
    function update($id)
    {
        ...
        {
            $this->load->model('Menumodel');
            if($this->Menumodel->returnbooledit($this->session->userdata('role'), 'pengguna'))
            {
                $data['akses']=$this->Menumodel->getstatus($this->session->userdata('role'), 'pengguna');
                $data['menu']=$this->Menumodel->getmenu($this->session->userdata('role'));
                ...
                else
                {
                    //meload file model
                    $this->load->model('Usermodel');
                    if($this->Usermodel->returnbooldata($id, $this->session->userdata('plant')))
                    {
                        //menjalankan fungsi ubah tampil
                        $data['hasil'] = $this->Usermodel->ubah_tampil($id);
                    }
